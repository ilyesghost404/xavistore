...
<head>
  ...
  <title>Produits Neufs - XaviStore</title>
...
<body>
  ...
  <section class="products-section">
    <div class="container">
      <h1>Produits Neufs</h1>
      <div class="products-grid">
        <?php
        $stmt = $pdo->prepare("SELECT * FROM produits WHERE categorie = 'neuf' AND etat != 'indisponible'");
        $stmt->execute();
        ...
        foreach ($produits as $produit): ?>
          <div class="product-card">
            ...
            <div class="product-details">
              ...
              <a href="product_detail.php?id=<?= $produit['id'] ?>" class="btn btn-primary">Voir Détails</a>
              <button class="btn btn-secondary" onclick="addToCart(<?= $produit['id'] ?>)">Ajouter au Panier</button>
            </div>
          </div>
        <?php endforeach; ?>
      </div>
  ...
  <!-- Silent login check and AJAX call -->
  <script>
